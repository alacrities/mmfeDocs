...
int IicProgClocks()
{
	u32 Index;
	int Status;
	int choice, i2c_choice, exit_flag, exit_function;
	...
	//   i2c_intr_id = XPAR_INTC_0_IIC_10_VEC_ID;
	   ...
	   while(exit_function != 1) {
	   xil_printf("Choose the Clock Generator:\n\r");
	   xil_printf("1: Clock Generator 0\n\r");
	   xil_printf("2: Clock Generator 1\n\r");
	   xil_printf("0: Exit\n\r");
	   do {
	       i2c_choice = inbyte();
	   ...
	   switch(i2c_choice) {
	    case '0':
	 	   xil_printf("you say 0\n\r");
		exit_function = 1;
	   		xil_printf("Exit\n");
	   	   	break;
	   	case '1':
	 	   xil_printf("you say 1\n\r");
	   		i2c_bus_id = XPAR_AXI_IIC_0_DEVICE_ID;
	   		i2c_intr_id = XPAR_INTC_0_IIC_0_VEC_ID;
	   		i2c_address = ICS8N4Q001_ADDRESS;
	   		break;
	   	case '2':
	 	   xil_printf("you say 2\n\r");
		i2c_bus_id = XPAR_AXI_IIC_0_DEVICE_ID;
	   		i2c_intr_id = XPAR_INTC_0_IIC_0_VEC_ID;
	   		i2c_address = ICS8N4Q001_ADDRESS;
	   		break;
	   	default:
	 	   xil_printf("you say other\n\r");
	   ...
	   exit_flag = 0;
	   while ((exit_flag != 1) & (exit_function != 1)) {

		xil_printf("Choose Operation:\n\r");
		xil_printf("1: Read Clock Settings\n\r");
		xil_printf("2: Write Clock Setting\n\r");
		xil_printf("0: Exit\n\r");
		do {
   		choice = inbyte();
       	if (isalpha(choice)) {
...
//				xil_printf("\r\n");
				...
				for (Index = 0; Index < 24; Index++) {
					if ((Index > 0) & ((Index % 4) == 0)) {xil_printf(" ");}
					xil_printf("%02x", RegisterSpace[Index]);
				}
				xil_printf("\r\n");

				for (def_freq_num = 0; def_freq_num < 4; def_freq_num++) {
					MINT[def_freq_num] = ((RegisterSpace[0+def_freq_num]>>1) & ~(~0 << 5));
					MFRAC[def_freq_num] = (((RegisterSpace[0+def_freq_num] & ~(~0 << 1))<<17) | RegisterSpace[4+def_freq_num]<<9 | RegisterSpace[8+def_freq_num]<<1 | ((RegisterSpace[12+def_freq_num] & (~0 << 7))>>7));
					N[def_freq_num] = (RegisterSpace[12+def_freq_num] & ~(~0 << 7));
					P[def_freq_num] = 0x01;
					f_out[def_freq_num] = (f_xtal * (MINT[def_freq_num] + (MFRAC[def_freq_num]/262144.0))/(P[def_freq_num]*N[def_freq_num]));
					printf("F%d = %4.2f MHz\n", def_freq_num,(f_out[def_freq_num]/1.0e6));
					}
				printf("\n");
...
//				xil_printf("\r\n");
				...
				xil_printf("Which default frequency do you want to change:\n");
				xil_printf("0: Default frequency 0\n");
				xil_printf("1: Default frequency 1\n");
				xil_printf("2: Default frequency 2\n");
				xil_printf("3: Default frequency 3\n");
				scanf("%x", &def_freq_num);

			if ((def_freq_num == 0) | (def_freq_num == 1) | (def_freq_num == 2) | (def_freq_num == 3)) {
				xil_printf("Enter the new frequency (in MHz):");
				scanf("%f", &freq);
//				freq = freq * 1e6;
//				printf("Freq = %4.2f MHz\n", (freq/1.0e6));

				found_N = 0;
				N_prog = 0;
				if ((found_N != 1) & ((2 * freq) >= 1950) & ((2 * freq) <= 2600)) {N_prog = 2; found_N = 1;}
				if ((found_N != 1) & ((3 * freq) >= 1950) & ((3 * freq) <= 2600)) {N_prog = 3; found_N = 1;}
				if ((found_N != 1) & ((4 * freq) >= 1950) & ((4 * freq) <= 2600)) {N_prog = 4; found_N = 1;}
				if ((found_N != 1) & ((5 * freq) >= 1950) & ((5 * freq) <= 2600)) {N_prog = 5; found_N = 1;}
				for (Index = 6; Index < 127; Index=Index+2) {
					if ((found_N != 1) & ((Index * freq) >= 1950) & ((Index * freq) <= 2600)) {N_prog = Index; found_N = 1;}
				}
				if (found_N != 1) {N_prog = 0;}
//				printf("N_prog = %02x\n", N_prog);
...
//				printf("MFRAC_prog = %05x\n", MFRAC_prog);
				...
				RegisterSpace[0+def_freq_num] = (RegisterSpace[0+def_freq_num] & 0xC0) | (MINT_prog<<1 & 0x3E) | (MFRAC_prog>>17 & 0x00000001);
				RegisterSpace[4+def_freq_num] = (MFRAC_prog>>9 & 0x000000FF);
				RegisterSpace[8+def_freq_num] = (MFRAC_prog>>1 & 0x000000FF);
				RegisterSpace[12+def_freq_num] = (MFRAC_prog<<7 & 0x00000010) | N_prog;
